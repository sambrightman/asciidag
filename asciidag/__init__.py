...
#     unsigned short default_column_color
    ...
    def strbuf_write_column(self, sb, c, col_char):
        if c.color is not None:
            strbuf_addstr(sb, self.column_colors[c.color])
        strbuf_addch(sb, col_char)
        if c.color is not None:
            strbuf_addstr(sb, self.column_colors[-1])
    ...
    def interesting_parents(self):
        ...
        for parent in parents:
            if self.is_interesting(parent.item):
                yield parent
            if self.revs.first_parent_only:
                break

    def get_current_column_color(self):
        if not self.revs.diffopt.use_color:
            return None
        return self.default_column_color

    def increment_column_color(self):
        self.default_column_color = (self.default_column_color + 1) % len(self.column_colors)
    ...
    def output_pre_commit_line(self, sb):
        ...
        for i in range(self.num_columns):
            ...
            if col.commit == self.commit:
                ...
                if self.prev_state == GraphState.POST_MERGE and self.prev_commit_index < i:
                        strbuf_write_column(sb, col, '\\')
                else:
                        strbuf_write_column(sb, col, '|')
                chars_written += 1
    ...
    def draw_octopus_merge(self, sb):
        ...
        for i in range(self.num_columns + 1):
            ...
            if col_commit == self.commit:
                ...
                if self.num_parents > 2:
                    chars_written += self.draw_octopus_merge(sb)
    ...
    def output_post_merge_line(self, sb):
        ...
        for i in range(self.num_columns + 1):
            ...
            if col_commit == self.commit:
                ...
                par_column = self.find_new_column_by_commit(next(parents).item)
                ...
                for parent in parents:
                    ...
                    strbuf_write_column(sb, par_column, '\\')
                    strbuf_addch(sb, ' ')
                chars_written += self.num_parents * 2
            elif seen_this:
                strbuf_write_column(sb, col, '\\')
                strbuf_addch(sb, ' ')
                chars_written += 2
